...
public class SignupPage {
	...
	public static final String SIGNUP_TITLE_XPATH = "//h4[contains(text(),'Welcome to the AUCDT')]";
	...
	public static final String MOBILE_NUMBER_NAME="contact-address";
	...
	public static final String HEAR_ABOUT_US_XPATH="//span[text()='Select how you heard about us']";
	...
	public static void signup(String firstName,String middleName,String lastName,
			String programme,String gender,String country,String hearAboutUs,String mobile,String email,String password,String confirmPassword,String captcha){
		...
		if(!("".equals(hearAboutUs))){
			SeleniumUtil.scrollToWebElement(By.xpath(HEAR_ABOUT_US_XPATH));
			SeleniumUtil.getWebElement(By.xpath(HEAR_ABOUT_US_XPATH)).click();
			SeleniumUtil.wait(2000);
			SeleniumUtil.getWebElement(By.xpath(SEARCH_LABEL_XPATH.replace("index", "4"))).sendKeys(hearAboutUs);
			SeleniumUtil.getWebElement(By.xpath(SEARCH_LABEL_XPATH.replace("index", "4"))).click();
			SeleniumUtil.wait(1000);
			SeleniumUtil.getWebElement(By.xpath(SELECT_LABEL_XPATH.replace("selectText", hearAboutUs))).click();
		}
		...
		if(!("".equals(mobile))){
			...
			SeleniumUtil.getWebElement(By.name(MOBILE_NUMBER_NAME)).sendKeys(mobile.substring(3,mobile.length()));	
		}
		
		if(!("".equals(email))){
			if("auto".equalsIgnoreCase(email)){
				Date date= new Date();
				 emailAuto="automation"+date.getTime()+"@aucdt.edu.gh";
				 System.out.println("**********Print Email*****"+emailAuto);
				SeleniumUtil.getWebElement(By.name(EMAIL_NAME)).sendKeys(emailAuto);	
			
			}else if("SignupUser".equalsIgnoreCase(email)){
				LOGGER.info("Inside alreadySingup user email if condition");
				SeleniumUtil.getWebElement(By.name(EMAIL_NAME)).sendKeys(emailAuto);
		...
		SeleniumUtil.PassTestStep(SeleniumUtil.getDriver(), Hooks.getExtentTest(), "Signup enter data successfully");
	...
	public static void ValidateErrorMessage(String message){
		...
		SeleniumUtil.PassTestStep(SeleniumUtil.getDriver(), Hooks.getExtentTest(), "Signup message validation successfully");
	}
	...
	public static void selectCountry(String country){
		  SeleniumUtil.validateWebElementVisible(By.xpath(COUNTRY_OF_RESIDENCE_XPATH), SeleniumUtil.waitWebElementSync);
			...
			SeleniumUtil.PassTestStep(SeleniumUtil.getDriver(), Hooks.getExtentTest(), "Signup select country successfully");
	}
}
