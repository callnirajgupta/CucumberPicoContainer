...
import org.example.pageobjectmodel.ContactInformationPage;
...
public class PersonalDetailsStepDefinition {
	...
	public PersonalDetailsStepDefinition(SeleniumUtil seleniumUtil){
		...
		for (int i = 0; i < dataTable.size(); i++) {
			...
			try {
				...
				Assert.assertEquals("The Error message is Not matching ", dataTable.get(i).get("Message"),
						PersonalDetailsPage.getErrorMessage());

			} catch (AssertionError e) {
				list.add(dataTable.get(i).get("Message"));
				flag = false;
				LOGGER.error("The Error message is Not displaying as " + dataTable.get(i).get("Message"));
				seleniumUtil.failTestStep(seleniumUtil.getDriver(), Hooks.getExtentTest(),
						" Error message is not matching");
				if (seleniumUtil.getWebElements(By.xpath(ContactInformationPage.CONTACT_INORMATION_TITLE_XPATH))
						.size() > 0) {
	...
	public void validateThatPersonalDetailsFilledDataPersist() {
		...
		try {
			Assert.assertTrue("The Photo is not matching", PersonalDetailsPage.validateuploadedPassportPhoto()
					.contains(PersonalDetailsStepDefinition.PersonalDataTable.get(0).get("Image")));
		} catch (AssertionError e) {
			e.printStackTrace();
			flag = true;
		}
		...
		try {
			// String[] DOB =
			// PersonalDetailsStepDefinition.PersonalDataTable.get(0).get("DOB").split("/");
